...
const Analytics = () => {
  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    for (let i = 29; i >= 0; i--) {
      dayMap.set(format(subDays(new Date(), i), "MMM dd"), 0);
    }

    visits.forEach((visit) => {
      const key = format(new Date(visit.visited_at), "MMM dd");
      dayMap.set(key, (dayMap.get(key) || 0) + 1);
    });

    setVisitsByDay(
      Array.from(dayMap.entries()).map(([date, visits]) => ({
        date,
        visits,
      }))
    );
    ...
    visits.forEach((visit) => {
      hustleMap.set(
        visit.hustle_id,
        (hustleMap.get(visit.hustle_id) || 0) + 1
      );
    });
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground">
          Track how your hustles are performing
        </p>
      </div>
      ...
      <div className="bg-card rounded-xl border border-border p-6">
        ...
        {hustleStats.length > 0 ? (
          ...
          <p className="text-muted-foreground text-center py-8">
            Create a hustle to see analytics
          </p>
        )}
      </div>
    </div>
